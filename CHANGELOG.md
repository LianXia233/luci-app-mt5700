...
桌面端（>=769px）分毫不动：所有新增规则都包在 `@media` 内，
集中追加在 `mt5700.css` 末尾，不删改任何既有规则，靠「同优先级后写覆盖」生效。

### 改进 - 四张环形仪表盘改 2x2 网格（4 屏 -> 1 屏）

原每行仅 1 张，单卡约 350px 高，四个信号指标需滑 4 屏；整页高达 7501px。
现改为 2x2：圆环 `120px -> 78px`（<=419px 时 68px）、内边距与间距收紧、
档位胶囊与描述行降档。「信号质量」卡片 `1425px -> 767px`（-46%）。

### 修复 - 宽表格在手机上必须横向拖动

载波聚合表 9 列、桌面宽 610px，而手机容器仅 288px，
右侧 RSRP / RSRQ / SINR 三列完全不可见。
<=600px 时改为**卡片式堆叠**：隐藏表头，每行一张小卡，
单元格用 `::before` 显示列名（列名由 JS 写入 `data-label`）。
表宽 `610px -> 294px`，9 列全部可见，零横向溢出。

**例外**：只有两列的键值表（IP 与 DNS / 连接诊断）不堆叠——
堆叠会把每项拆成上下两行，项数一多反而更高（实测一度 `871px -> 1023px`）。
JS 侧检测 `headers.length === 2` 时加 `.mt5700-table-kv` 类，
走「标签左 / 值右」紧凑分栏行，实测回落到 `599px`。

### 修复 - 文案与数值在窄屏被截断

- `.mt5700-metric-value` 原为 `white-space:nowrap` + `ellipsis`，
  窄屏长值被裁：「64QAM MCS 20 · 1 层」填充率 **1.46**、
  「2500 MHz (TDD)」**1.34**、「已注册（本地网络）」**1.24**。
  手机端改为允许换行，并压低 `len-md/len-lg/len-xl` 降字号档位。
- 「网络能力」指示器的指标名与描述原为 **8px**（低于可读下限），
  提到 **11px** 并允许换行；指标区改 2x2，避免 4 列挤成小字。

### 改进 - 触屏可用性

去掉触屏上无意义的 `:hover` 抬升（按下不再抖动）；按钮最小高度提到 32px。

### 重构 - 表格渲染函数支持列名前缀

`Mt5700.table()` 现在把表头文案写入每个 `td` 的 `data-label`，
供移动端 `content: attr(data-label)` 读取；桌面端不显示，零影响。
两列表格额外加 `.mt5700-table-kv` 标记类。

### 验证

- 手机三档视口（390x844 / 360x800 / 320x720）量化对拍：
...
- PC 五档（1920 / 1600 / 1440 / 1280 / 1200）全部 PASS：
  四圆环严格同一行（垂直中心偏差 <= 3px）、`viewBox 0 0 100 100`、
  每张 2 个 `circle`、数值/单位/标签/档位齐备、零横向溢出、零 JS 报错。
- 四张环形仪表盘样式与数值**未被移动端规则影响**（桌面端规则完全未改）。
- 实机 SHA256 与本地逐字节一致（3 个前端文件），零 CRLF。

## [1.11.1] - 2026-09-13

本版修两类问题：**① 网络能力指示器的指标图标绘制方式与卡片几何**；
**② 载波聚合卡片的信号三项读取补全**。均不改任何评估逻辑与阈值。
...
### 修复 - 载波聚合卡片信号三项恒为 `—`（信息读取补全）

用户反馈：5G 页「载波聚合」卡片的 RSRP / RSRQ / SINR 三列全是 `—`。
排查后确认是**两个独立缺陷叠加**：

**根因 1 — 硬编码 `null`**：`network_status.js` 在把 `^HFREQINFO` 结果映射成
`state.carriers` 时，把 `pci / rsrp / rsrq / sinr` 四项**写死为 `null`**。
而 `^HFREQINFO` 本身**只返回频点 / 频率 / 带宽，不含任何信号字段**，
信号实际来自 `^MONSC` 的 `serving`。写死 `null` 后前端只能渲染 `—`。

**根因 2 — 两套频点体系不同，守卫条件恒不成立**：第一版修复试图用
「`serving.channel == carriers[0].dlFcn`」判断同小区，仍然失败。实机实测两条
AT 返回的是**两套不同的频点编号**：`^HFREQINFO` 的 `dlFcn=513000` 是 NR-ARFCN
（5 kHz 栅格，对应 2565 MHz）；`^MONSC` 的 `channel=149002` 是另一套编号
（按 15 kHz 栅格反推为 -3765 MHz，不合理）。用「频点相等」做判据必然恒为假。

修复：改用**同制式**守卫（`^HFREQINFO` 的第 0 个载波协议上即主载波）：
...
仅对**主载波（`idx === 0`）**回填，辅载波仍留 `—`（`^MONSC` 不提供辅小区信号），
避免把主小区数值复制到辅载波上造成误导。

### 改进 - 载波聚合表格补「角色」「下行频率」两列与空态提示

- 表头 7 列 → 9 列：新增**角色**（主载波 / 辅载波 N）与**下行频率**
  （由 `dlFreqKHz` 换算为 MHz），原「带宽」列保留。
- 新增**空态提示**：`^HFREQINFO` 无返回时显示「未查询到激活载波」，
  不再渲染一张只有表头的空表。
- 新增**单载波说明**：只有 1 个激活载波时提示「当前仅 1 个激活载波（未做
  载波聚合）。信号三项取自 ^MONSC 主小区，与上方仪表盘同源。」，
  避免用户误以为是缺陷。
...
- 载波聚合卡片实机实测（JS errors: none）：
  `主载波 | NR | 2500 MHz (TDD) | 513000 | 2565 MHz | 100 MHz | -63 dBm | -10 dB | 29 dB`，
  与上方仪表盘数值同源一致
