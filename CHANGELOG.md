...
### 改进 - 网络能力横幅完整复刻参考稿单行横向布局

对齐设计稿 `mt5700_dynamic_svg_all_states_preview.html`，
把原先「结论 → chip 徽标组 → 能力项竖排」三层堆叠，重写为**单行横向流**：

```
◉ │ 网络能力优秀        │ [▮信号强度 优秀][∿信号质量 优秀] │ 网络制式 5G  │
  │ 信号与频段带宽俱佳  │ [◎信噪比 优秀]  [✓综合 优秀]     │ 频段特性 …   │
  31px      205~340px         flex:1（四列网格）              能力三列
```

- **状态图标**（新增 `statusIcon()`）：31px，外圈淡环 `.mt5700-status-halo`
  + 内实心圆 `.mt5700-verdict-tower`（带 `mt5700-tower` 脉冲动画）。
- **摘要块**：标题 `16px/700` + 描述 `10px` muted 单行省略。
- **四指标网格**：`repeat(4, minmax(75px, 1fr))`，每项 `23px` 描边 SVG
  + 竖排 `9px` 标签 / `12px` 加粗值，带 `1px` 边框小卡。
- **能力三列**：`border-left` 细竖线分隔，标签 `9px` / 值 `12px`，
  `max-width:115px` + `ellipsis`。
- **一个变量管全部**：`--mt5700-verdict-accent` 由档位类统一设置，
  图标 / 数值 / 左边条全部继承 `currentColor`，无需逐元素写色值。

### 修复 - 描述文案被截断（`scrollWidth/clientWidth` 由 1.67 降到 1.00）

原来的中文描述比设计稿长 55%，而摘要区写死 `205px`，
实测填充率 **1.67**（342/205，截掉约 2/3）。双重修复：

1. **文案缩短至设计稿量级**（`exc` 由 34 字 → 22 字）：
   `信号与频段带宽俱佳，适合高清视频及大文件下载`
2. **摘要区宽度按视口分级**，让 10px 中文整句放得下：

| 视口 | 摘要宽度 | 填充率 | 截断 |
|:--|:--|:--|:--|
| `>= 1400px` | `340px` | **1.00** | 否 |
| `1100-1399px` | `240px` | **1.00** | 否 |
| `901-1099px` | `168px` | 1.32 | 省略号 |
| `<= 900px` | `155px` | 1.43 | 省略号 |
| `<= 520px` | `100px` | **1.00** | 否（换两行） |

作为对比，设计稿本身在 **1920 / 1440 / 1280 / 1200 / 1024px 全部截断**
（固定 205px、填充率恒为 1.073）。本实现把这些宽度压到 1.00，
整句完整可读；仅在 901-1099px 与 <=900px 这两个真正拥挤的区间才用省略号。

### 修复 - 底部说明条与指标区重叠

`flex: 1 1 100%` 未生效导致说明条叠在指标卡上。给容器加 `flex-wrap: wrap`，
说明条改 `flex: 0 0 100%; width: 100%; align-self: stretch`，
实测 `noteTop(487) >= metricsBottom(472)`，不再重叠。

说明条两种来源（`flex: 0 0 100%` 独占一行，默认隐藏）：

1. **结论被能力维度拉低** → 点明「信号好 != 网速快」及具体短板；
2. **信号本身偏弱**（`bad` / `poor`）→ 新增 `VERDICT_TEXT[level].hint` 字段，
   给出可操作的排查建议（如「建议把设备移到靠窗或高处、加装外置天线」）。

### 改进 - 动态 SVG 图标完整移植设计稿 7 组动画

| 图标 | 类名 | 动效 |
|:--|:--|:--|
| 信号强度 | `.mt5700-bar` x 4 | `mt5700-bar-pulse` 逐柱脉冲 |
| 信号质量 | `.mt5700-wave` | `stroke-dashoffset` 波形流动 |
| 信噪比 | `.mt5700-ring` x 2 + `.mt5700-sweep` | 同心环扩散 + 指针旋转 |
| 综合 | `.mt5700-check` | 对勾描边勾勒 |
| 状态图标 | `.mt5700-verdict-tower` | `mt5700-tower` 圆点脉冲 |

`prefers-reduced-motion` 下全部动画关闭。实测四项图标动画数 `[4, 1, 3, 1]`，
状态图标动画 `mt5700-tower`。

### 修复 - 短板高亮只在真正唯一短板时点亮

信号指标与能力项同一策略：要求 `cappedBy` 非空 **且** 最差档只有一项
（`worstCount === 1`）才加高亮。四项同为 `exc` 时不再全亮，保留指向性。
...
- 排版断言 **18/18 通过**（1920 / 1440 / 1200 / 1024 / 900 / 768 / 520 / 390）
- 十档视口横向溢出复测：**全部无溢出**（`scrollWidth == clientWidth`）
- 描述填充率对拍设计稿：**1920/1600/1440/1280/1200 五档均为 1.00**，优于设计稿
- 全站 12 页回归通过（零 JS 报错、零 4xx/5xx）
- 四圆环未受影响：仍为 4 张、SVG 仍 2 个 `circle`、`viewBox` 仍 `0 0 100 100`
- 明暗双模式实测取色正确（浅色 `#10a56a` / 深色 `#34d399`）
- 实机 SHA256 与本地逐字节一致（`mt5700.js` / `mt5700.css` / `rpc.js` / `network_status.js`）

## [1.10.0] - 2026-09-13

本版**只改布局，不改功能**。修掉三处自适应缺陷：大屏两侧留白过大、
指标网格列数写死导致不够宽就挤、仪表卡高度溢出压住底部按钮。
同时理顺信号总评横幅的文字层次，让标题与说明不再视觉粘连。

### 修复 - 指标网格高度溢出，压住下方「查看指标说明」按钮

`grid-template-columns` 按视口宽度写死列数，但**内容区实际宽度由外层主题
容器决定**。以 1440 视口为例：

```
外层主题容器 max-width:1280px
  → 内容区实际可用宽 1116px
  → 强制 4 列，每列被压到 270px
  → 仪表卡内容需要 328px 高
  → 网格行高只有 294px  ← 溢出 34px
  → 溢出的卡片盖住下方「查看指标说明」按钮
```
...
| 指标 | 实测值 |
| --- | --- |
| `.mt5700-metrics` 行高 | 294px |
| `.mt5700-gauge` 实际高 | 328px |
| `grid.scrollHeight` | 328 > `clientHeight` 294 |
| 底部按钮中心点命中测试 | 落在卡片子元素上（被遮挡） |

修法：

- 网格改用 `repeat(auto-fit, minmax(200px, 1fr))` + `grid-auto-rows: 1fr`，
  列数与行高均由容器实际宽度驱动，不再依赖视口；
- 仪表卡由 `height: 100%` 改为 `min-height: 100%` + `box-sizing: border-box`，
  内容再高也只是自身撑开，不再溢出网格；
- 新增 `.mt5700-metrics > * { align-self: stretch; }` 保证同行等高。

修复后实测：**溢出 0px，按钮间隙稳定 16px，命中测试 CLICKABLE**。

### 修复 - 大屏右侧留白过大（内容区不自适应）

`.mt5700-page` 固定 `max-width: 1400px`，在 1920 视口下先被 `1400px` 限制、
再被外层主题的 `1280px` 收一道，最终内容区只剩 1248px，**两侧各空 336px**。

| 视口 | 修复前内容宽 | 修复前右留白 | 修复后内容宽 | 修复后右留白 |
| --- | --- | --- | --- | --- |
| 1920 | 1248px | 336px | 1646px | 37px |
| 1440 | 1248px | 96px | 1166px | 37px |
| 1366 | 1248px | 59px | 1106px | 37px |

修法：页面容器改为 `width: 100%; max-width: none`，铺满父容器可用宽度；
仅在 `>= 1680px` 才用 `max-width: 1760px` 收口，避免超宽屏下行宽过大影响阅读。

### 修复 - 网格列数不随容器宽度自适应

原实现按视口宽度写死列数（`min-width:960px` 强制 4 列、`min-width:1500px`
强制 5 列），列数与「容器实际可用宽度」脱钩，窗口一窄就挤成一团。

修复后列数完全由 `auto-fit` 计算，实测各视口表现：
...
只保留两档小屏兜底（`<=719px` 两列、`<=419px` 单列），以及 `2/3` 项时的
防拉伸规则——项数少时若铺满整行单卡会宽到不协调。

### 优化 - 信号总评横幅文字层次

标题与说明的 `margin-top` 原为 `3px`，长标题换行后两者几乎贴在一起；
chip 组也紧接在说明下方。调整：
...
| 标题 → 说明 | 3px | 8px |
| 说明 → 指标 chip | 8px | 18px |
| 横幅内边距 | `md` 单值 | `md` / `lg` 横纵分离 |
| 说明行高 | 1.6 | 1.7 |

标题 `line-height` 由 `1.4` 提到 `1.5`，多行标题更易读。

### 验证

- 布局断言 **22/22 通过**（1920 / 1440 / 1024 / 390 四档视口，
  断言项：网格无溢出、各项等高、按钮不被压、文案间距达标、宽屏留白收紧）
- 全站 **12 页回归全部通过**（零 JS 报错、零 4xx/5xx）
- 实机文件 SHA256 与本地逐字节一致

### 注意

- 本版**未改动任何功能逻辑与后端参数**，仅 CSS 布局与间距。
- 页面容器改为铺满后，超宽屏（>=1680px）下卡片会明显变宽，
  若偏好窄栏阅读可在主题侧自行加 `max-width` 覆盖。

## [1.9.0] - 2026-09-13

本版修掉一个**把载波信息整体读错位**的解析缺陷，并重做网络质量评估逻辑：
不再"信号好就下结论说网速快"，而是把**网络制式、频段特性、载波带宽**三个决定
实际吞吐的维度一并纳入判断。同时修正信号质量卡片四项指标的排版对齐。

### 修复 - `^HFREQINFO` 字段解析整体错位（载波表「制式」列显示 `0`）

`parseHFREQINFO()` 把 9 字段的返回串按 8 字段解析，导致**每一个字段都错位一格**：

```
手册 13.16.1 实际顺序：
  ^HFREQINFO:<n>,<sysmode>,<band_class>,<dl_fcn>,<dl_freq>,<dl_bw>,<ul_fcn>,<ul_freq>,<ul_bw>

实测（NR n41）：
  0, 7, 41, 513000, 2565000, 100000, 513000, 2565000, 100000
  n=0  sysmode=7(NR)  band=41  dl_fcn=513000  dl_freq=2565000kHz(2565MHz)
  dl_bw=100000kHz(100MHz)  ul_fcn=513000  ul_freq=2565000kHz  ul_bw=100000kHz
```

| 表格列 | 修复前显示 | 修复后显示 | 原因 |
| --- | --- | --- | --- |
| 制式 | `0` | `NR` | 误把 `<n>` 当制式 |
| 频段 | `Band 7` | `2500 MHz (TDD)` | 误把 `<sysmode>` 当频段号 |
| 频点 | `41` | `513000` | 误把 `<band>` 当频点 |
| 带宽 | `513000` | `100 MHz` | 误把 `<dl_fcn>` 当带宽，且未换算 KHz→MHz |

更严重的是，旧代码还从该命令解析 `RSRP/RSRQ/SINR`——而 **`^HFREQINFO` 根本不返回信号字段**
（手册 13.16.3 无此定义），于是把 `dl_bw/ul_fcn/ul_freq` 当成信号值做 `convert*` 换算，
得到 `-44 dBm / -3 dB / 30 dB` 这种"完美但虚假"的信号读数。现改为：
**信号质量一律取自 `^MONSC`，载波表只呈现频率与带宽，并移除不存在的 PCI 列。**

- `rpc.js`：`parseHFREQINFO()` 按手册重写为 `{n, sysModeCode, sysMode, band, dlFcn,
  dlFreqKHz, dlBwKHz, ulFcn, ulFreqKHz, ulBwKHz}`；新增并导出 `sysModeName(code)`
  （`1`GSM / `3`WCDMA / `6`LTE / `7`NR，文档 13.16.3；**注意与 `^SYSINFOEX` 的
  `6`LTE / `11`NR-5GC 编码不同，不可混用**）。
- `network_status.js`：载波映射同步更新；带宽统一按 KHz→MHz 显示；载波表与辅载波表
  均移除 PCI 列（该列数据源不存在）。「连接状态」卡片新增「主载波频段」「主载波带宽」两项。

> 顺带核对结论：`parseMONSC()`（格式 A，NR）与 `parseHCSQ()`（NR 分支）经实机
> 交叉验证**解析正确，无需修改**——`^MONSC: NR,460,00,504990,1,C2840C002,80,149002,-63,-9,28`
> 与 `^HCSQ: "NR",77,236,31` 两条独立命令换算后完全吻合（`-63/-9/28` vs `-63/-4/27.2`）。

### 变更 - 网络质量评估改为「信号 + 承载能力」双维度

原逻辑只取 `RSRP/RSRQ/SINR/信号百分比` 四项中最差档位下结论。这在**信号好但承载受限**
时会产生明显乐观误判：700MHz 上的 LTE 信号满格同样判"优秀"，进而建议
"适合看高清视频、下载大文件"——而 20MHz LTE 单载波的峰值吞吐远达不到该场景要求。
...
**频段特性档位**（按实测下行频率划分，而非频段号——频段号映射随 3GPP 版本扩展，用频率更稳）：
...
> 关于 n78(3.5GHz)：它是联通/电信/广电的 5G 核心频段，100MHz 带宽的实际速率**优于**
> 低频段，因此归入"中频/优秀"，不因"频率高"而降档。
...
**结论文案**同时改为"网络能力"口径（如"网络能力优秀"而非"信号很好"），并新增一条
**仅在结论被能力维度拉低时才出现**的提示条，明确指出是哪个维度拖后腿：

> 注意：无线信号本身优秀，但「载波带宽」为较差（带宽偏窄，峰值明显受限），实际网速会受此限制。

- `mt5700.js`：新增 `api.capabilityAssess({sysMode, dlFreqKHz, dlBwKHz})`、
  `api.ratLabel(v)`（`NR`→`5G`、`LTE`/`6`→`4G`，兼容数字码与字符串两种制式表示）、
  `api.levelCaption(level)`；`api.signalVerdict()` 的 `set()` 扩展为接收
  `{rsrp, rsrq, sinr, pct, sysMode, dlFreqKHz, dlBwKHz}`。
- 评估器对缺失维度**自动跳过**（如某制式无带宽上报时不参与汇总），不会因数据不全而误判。

### 修复 - 信号质量卡片四项指标排版对齐

四个仪表的描述文案长度不一，导致卡片高度参差、各行基线不齐：

- `.mt5700-gauge` 增加 `height: 100%`，四卡在网格内等高。
- `.mt5700-gauge-labelbox` 增加 `min-height: 2.6em`，中英文双行标签占位固定。
- `.mt5700-gauge-plain` 改为 `flex` + 固定 `height: 4.5em` 居中，描述行不再随文案伸缩。

实机几何验证（`getComputedStyle` 回读）：四卡 `top/bottom/height` 完全一致
（`643 / 971 / 328`），标签行、档位行、描述行各自的 `top` 均严格对齐，仪表盘等宽 `120px`。

### 变更 - 其它

- `MT5700_CSS_VERSION`：`2.3.2` → `2.4.0`（破浏览器缓存）。
- 新增验证脚本断言：`.analysis/e2e_v19.py`（38 项）、`.analysis/e2e_verdict.py`（19 项，
  覆盖"信号满格但制式/带宽受限"的 7 个场景）、`.analysis/regress_v19.py`（全站 12 页）。

## [1.8.0] - 2026-09-13

本版把两处「只有专业用户看得懂」的界面翻译成普通用户能自主操作的形式：
**网络系统配置**（接入顺序等 5 项）与**信号质量**卡片。**后端参数与配置功能完全不变，仅呈现层改进。**

### 变更 - 网络接入顺序改为可自主选择的卡片控件

原实现是一个裸文本框，要求用户手拼 `080302` 这类制式码串，且占位符写的是
`0504030200（5G→4G→3G→2G）`——**这个示例本身就是错的**：`05` 在 MT5700 上不存在
（TD-SCDMA/05 未被该模组支持），`00` 也不是合法制式码。用户照抄会下发非法参数。

现改为**卡片式单选组**，取值严格限定在 `AT^SYSCFGEX=?` 返回的合法制式码
（`01` GSM / `02` WCDMA / `03` LTE / `08` NR）组合之内，每项都写清对网络连接的实际影响：

| 选项 | 下发码 | 实际影响 |
| --- | --- | --- |
| 5G 优先，逐级回落（默认） | `080302` | 有 5G 用 5G，无则落 4G，再落 3G |
| 仅 5G | `08` | 只搜 5G，搜不到持续重搜 |
| 4G 优先，可回落 3G | `0302` | 优先 4G，无 4G 落 3G |
| 仅 4G | `03` | 只搜 4G，耗电较低 |
| 3G 优先，可回落 4G | `0203` | 优先 3G，无 3G 落 4G |
| 仅 3G | `02` | 只搜 WCDMA |
| 保持当前设置 | `99` | 不修改接入顺序 |

- `modem_settings.js`：`ACQ_OPTIONS` 定义全部选项（含 `code` 角标展示原始码串）；
  模组当前值不在预设内时，**不静默改写**，而是显示提示条告知实际值，由用户决定是否修改。
- 新增「原始参数」只读面板（默认折叠）：把 5 个字段的原始十六进制值一并列出，便于排障对标 AT 手册。

### 变更 - 其余 4 个字段通俗化，并补齐官方支持的档位

| 字段 | 原界面 | 现界面 |
| --- | --- | --- |
| 2G / 3G 频段 | 裸文本框「频段位图，留空为全部」 | 预设下拉：不修改 / 自动 / 全部频段 / 当前组合 |
| 漫游 | 2 档（仅本网 / 自动漫游） | **3 档**补齐官方 `0`（禁止漫游），文案改为「允许漫游 / 禁止漫游 / 不修改」 |
| 服务域 | 3 档（仅电路域 / 仅分组域 / 电路+分组域） | **5 档**补齐官方 `3`（不限）与 `4`（不修改），文案通俗化 |
| 4G / LTE 频段 | 裸文本框「LTE 频段位图」 | 预设下拉：不修改 / 常用频段 / 全部 LTE 频段 |

- 频段与 LTE 频段的手写位图入口已移除——这两个值是十六进制位图（如 `2000000680380`），
  手写极易出错且无法校验，改为预设后普通用户不会下发非法值。
- **服务域互斥约束显性化**：官方手册 13.2.3 注 2 规定「设置的模式里含有 L 或 NR，
  服务域不允许设置为 0 或 3」。界面按当前接入顺序**动态禁用**这两项并给出提示，
  把原本只存在于文档里的约束直接呈现在界面上。

### 修复 - LTE 频段值被 `OK` 回显污染

`fetchSysCfg()` 的解析正则尾段为 `([^,]*)`，而 `AT^SYSCFGEX?` 的响应形如
`^SYSCFGEX:"080302",2000000680380,1,2,1E200000095\r\nOK`——`OK` 前面没有逗号，
于是被一并捕获，界面显示成 `1E200000095OK`。现解析前先剥离 `OK` 回显，
并对 LTE 频段字段用 `[0-9A-Fa-f]*` 收尾匹配。

### 变更 - 信号质量卡片增加通俗解读层

原卡片只有 4 个环形仪表，标签是 `RSRP` / `RSRQ` / `SINR` / `信号百分比`，
普通用户看完不知道「这信号到底能不能用」。现增加三层解读：

- **总评横幅**：把四项指标汇总成一句结论 + 可执行建议
  （如「信号很好 · 适合看高清视频、下载大文件」）。
  综合档位按**木桶原理取最差项**——任何一项拖后腿都会实际影响体验，取最差比取平均更贴近真实感受。
  横幅下方逐项列出各指标档位，并**高亮标出是哪一项拖后腿**。
- **中文主标签 + 英文副标签**：信号强度`RSRP` / 信号质量`RSRQ` / 信噪比`SINR` / 综合信号。
  普通用户看中文即可理解，专业用户仍能对照 AT 手册。
- **通俗描述行**：每个仪表下方补一句人话，如「信号很强，网速跑得动」「干扰严重，容易出现卡顿」。
- **可折叠的指标说明**：解释各指标含义与参考区间，供愿意深究的用户查阅。

### 修复 - 环形仪表弧长量程过窄导致视觉失真

RSRP 量程原为 `-120 ~ -70`，实机 `-64 dBm` 已被截断在满圈，与 `-80 dBm` 在视觉上
无法区分，而 `RSRQ -9` 却只显示大半圈——同是「优秀」档位却给出不一致的视觉信号。

量程按各指标物理可达范围重新标定（RSRP `-140 ~ -44`，SINR `-10 ~ 30`），
弧长百分比回归正常区分度。实测弧长（`stroke-dashoffset`，越小弧越长）：
`-64` → 55.0，`-85` → 112.7，`-118` → 203.4，五档单调且差异显著。

同时给仪器卡补上档位描边色（`mt5700-gauge-exc/good/fair/poor/bad`），
一眼扫过即可看出哪张卡处于异常档位。

### 新增 - 前端组件

- `Mt5700.radioCards(name, options, value, onChange)`：卡片式单选组，
  支持 `setValue(v, silent)` / `setDisabled(v, off)` / `setAllDisabled(off)`。
  用于选项数量少且每项都需要解释的场景。
- `Mt5700.rawValue(label, value)`：原始参数只读展示行。
- `Mt5700.fieldNote(summary, points)`：结构化字段说明块。
- `Mt5700.readonlyField(label, value, hint)`：只读字段。
- `Mt5700.signalVerdict()`：信号总评横幅，`set({rsrp, rsrq, sinr, pct})`。
- `Mt5700.gauge()` 扩展 `opts.sub`（英文副标签）与通俗描述行。

### 验证

- 端到端（Playwright + Chromium，实机 ImmortalWrt 路由器）：
  网络系统配置卡片 **26 项断言通过**（含服务域约束联动、原始参数面板、选中态样式差异）；
  信号质量卡片 **17 项断言通过**（含五档颜色唯一性、弧长单调性、木桶原理汇总、空值兜底）。
- 全站回归：12 个页面全部正常渲染，**零 JS 报错、零 4xx/5xx 资源请求**。
- 部署校验：4 个文件经 SFTP 上传后 SHA256 与本地一致，关键串逐项命中。
- 约束核验：`fetchSysCfg()` 函数体内 IMEI 相关命令数为 **0**（本轮未调用 `ATI` / `AT+CGSN` / `AT^PHYNUM`）。

## [1.7.0] - 2026-09-13

本版在两处做增强：温度分级由 3 档细化为 6 档（阈值整体下调、色彩更丰富），
并修复指标卡在窄列宽下的数值换行问题。**功能行为不变，仅呈现层改进。**

### 变更 - 模组温度分级由 3 档细化为 6 档

阈值整体下调，使常驻温度即可反映状态，异常升温能逐级显现：

| 温度区间 | 状态 | 颜色 | 含义 |
| --- | --- | --- | --- |
| `< 35 ℃` | `cold` | 深蓝 | 偏低（刚上电 / 低温环境） |
| `35 ~ 52.9 ℃` | `cool` | 蓝 | 温和 |
| `53 ~ 60.9 ℃` | `normal` | 绿 | 正常（长期工作舒适区） |
| `61 ~ 68.9 ℃` | `warm` | 黄 | 偏暖（关注） |
| `69 ~ 76.9 ℃` | `hot` | 橙 | 偏高（需散热） |
| `≥ 77 ℃` | `high` | 红 | 过高（告警） |

对照旧版 3 档（< 70 正常 / 70–84.9 偏高 / ≥ 85 过高），新阈值按 MT5700 实测区间
（各芯片常态 40–50 ℃）校准：常驻温度落在 `cool` / `normal` 档，40 ℃ 以下可识别为偏低，
60 ℃ 以上开始告警，较旧版提前约 10 ℃ 发出「偏暖」提示。

- `mt5700.js`：分级逻辑改为**阈值表驱动**。新增 `api.TEMP_LEVELS`（按 `min` 从高到低匹配），
  新增 `api.tempLabel(value)` 返回中文状态（偏低/温和/正常/偏暖/偏高/过高）。
  `api.TEMP_WARN_C` / `api.TEMP_HIGH_C` 保留为兼容字段，值同步为 `61` / `77`。
- `network_status.js`：`renderTemp()` 汇总逻辑改为**按严重度取最高档**
  （`TEMP_SEVERITY` 映射 0–5），卡片级强调仅「偏暖及以上」触发（`temp-has-warm/hot/high`）；
  新增卡片右上角徽标，显示「最高 X ℃ · 状态」，无数据时不显示。
- `mt5700.css`：温度样式块由 3 档替换为 6 档（每档含渐变背景、描边、阴影、数值同色），
  `temp-high` 保留呼吸光晕告警动画；旧的 `temp-warn` 相关规则与令牌已全部清除。
  浅色 / 暗色双主题各配一套色值，并遵循 `prefers-reduced-motion`。
- 单测：`tests/mock-modem/temp-level-test.js` 扩展至 **26 条断言**，覆盖 6 档全部边界
  （34.9/35、52.9/53、60.9/61、68.9/69、76.9/77）、无效值、1–120 ℃ 全程单调不降档、
  逐项独立分级、以及 6 档在 1–130 ℃ 区间内全部可达。

### 修复 - 指标卡数值异常换行

窄列宽下部分数值被拆成两行，影响可读性：

- **`102.40 Mbps` 断成 `102.40 Mbp` + `s`**：根因是 `.mt5700-metric-value` 上的
  `word-break: break-all` 允许在任意字符处断行。已改为 `word-break: normal` +
  `white-space: nowrap`，数值与单位不再拆行。
- **单位与数值同行但字号相同、层级不清**：新增 `api.metricValue()`，把「数值 + 单位」
  拆为两段渲染，数字用主色大字、单位弱化至 `0.62em`。单位白名单显式枚举（Mbps/Gbps/kbps/
  dBm/dB/MHz/GHz/ms/℃/%/GB/MB 等），避免把「5G」「NR」这类纯文本误拆。
- **中文长值（如「已注册（本地网络）」）易截断**：新增 `len-md` / `len-lg` / `len-xl`
  三档自适应降字号（按「显示宽度」估算，CJK 记 1、ASCII 记 0.55），保证单行完整显示。
- **尾行大片留白**：新增 `api.syncMetrics(grid)`，把实际项数写入 `data-count`，
  由 CSS 按项数选择最均衡列数（2→2 列、6→3 列、8→4 列、9→3 列、10→5 列等），
  消除「一行 5 个 + 尾行 2 个」的空白。
- 同时移除 `.mt5700-metric-label` 上的 `text-transform: uppercase`（中文标签无意义，
  且与参考站视觉规范不符），标签超长时改为省略号。

### 验证

- 部署校验：3 个文件 MD5 一致、22 项关键串抽检全通过、旧 `temp-warn` 残留为 0、
  静态资源可读（`mt5700.css` 53 KB / `mt5700.js` 28 KB / `network_status.js` 32 KB）。
- 端到端（Playwright + Chromium，实机 ImmortalWrt 路由器）：注入 6 档样本后
  **背景色 / 数值色 / 边框色各 6 个唯一值**，无效值不着色，卡片汇总取最高档 `high`，
  徽标显示「最高 81.0 ℃ · 过高」；真实数据各项类名正确。
- 排版回归：1440 / 834 / 414 px 三种视口下，所有指标卡数值**单行、无截断、单位同行**。
- 控制台无 JS 报错（`/cgi-bin/luci/` 未认证跳转产生的 403 为 LuCI 预期行为，与本插件无关）。

## [1.6.0] - 2026-09-13

本版聚焦前端视觉规范统一与模组温度可视化：全站设计令牌对齐 PaperGrid 冷调青色体系，
并为「模组温度」卡片新增按温度状态动态着色的能力。**功能行为不变，仅视觉与呈现增强。**

### 新增 - 模组温度按状态动态着色

针对 `admin/modem/5g/status`（网络状态）页的「模组温度」卡片，各芯片温度按自身数值
**逐项独立着色**，不再是一律中性底色：
...
- `mt5700.js`：新增 `api.tempLevel(value)` 与 `api.tempClass(value)`，并导出阈值常量
  `api.TEMP_WARN_C = 70` / `api.TEMP_HIGH_C = 85`。判定返回
  `normal` / `warn` / `high`；无效值（`null` / `NaN` / `<= 0`）返回 `null`，不着色。
- `mt5700.js`：`api.metric()` 增加可选第 4 参数 `status`（状态类名）。
  **向后兼容** —— 不传时行为与旧版完全一致，既有的 2/3 参数调用点无需改动。
- `network_status.js`：`renderTemp()` 改为逐项着色，并把「最严重的一项」汇总到卡片：
  存在过高项时卡片带 `temp-has-high`，仅有偏高项时带 `temp-has-warn`，用于快速定位过热源。
- `mt5700.css`：新增 `.mt5700-metric.temp-normal / .temp-warn / .temp-high` 渐变背景、
  描边、数值同色，及过高态的呼吸光晕动画；`.mt5700-card.temp-has-*` 卡片级边框强调。
  遵循 `prefers-color-scheme`（暗色下使用亮色系色值）与 `prefers-reduced-motion`（关闭动画）。
- **为何逐项判定而非取平均**：取平均会被正常项拉低而掩盖单点过热。例如 7 项分别为
  62/74/88/55/67/71/80 ℃ 时均值为 71 ℃（仅达「偏高」），但 MIMO PA 实际已达 88 ℃ 过热。
- 单测：`tests/mock-modem/temp-level-test.js` 覆盖三档边界（69.9/70/84.9/85）、
  无效值、以及逐项独立分级共 14 条断言，全部通过。

### 优化 - 视觉规范对齐 PaperGrid 冷调青色体系

参考站点（浅色主调、青色系强调色的博客主题）的设计令牌已对齐到 `mt5700.css` 的变量层，
**结构与组件类名保持不变**，仅调整视觉：

- 主色由蓝色系 `#3b82f6` 改为青色系 `#087cba`；暗色下翻转为亮青 `#61c9f4`。
- 背景/卡片/正文/边框改为冷调体系：浅色 `#f2f7fb` / `#fff` / `#17324b` / `#dce8f1`，
  暗色 `#0d1929` / `#132337` / `#e0edf7` / `#263d52`。
- 新增 `--mt5700-ease: cubic-bezier(0.22, 0.8, 0.25, 1)` 与
  `--mt5700-ring` 焦点环令牌，统一交互反馈曲线。
- 圆角基准对齐 `.85rem`（组件圆角整体收紧一档）。
- 阴影改为冷调低透明度投影，贴近参考站点的纸感层次。
- 环境背景光晕由蓝/绿/靛改为青/亮青/青绿三色。
- 组件细节同步：按钮悬停、输入框聚焦、开关/复选框焦点、Tab 激活、
  徽标底色、仪表盘光环与等级色、步骤条、终端提示符、日志配色。
- 全量清除 CSS 中的蓝色系硬编码色值（`rgba(59, 130, 246, *)` 等），统一走令牌。
- `MT5700_CSS_VERSION` 提升至 `2.1.0`，避免浏览器命中旧缓存。

### 验证

- 路由器实测（ImmortalWrt SNAPSHOT / aarch64）：注入三档温度后，
  `temp-normal` / `temp-warn` / `temp-high` 三类计算样式确认互不相同；
  真实数据 7 项温度 40.5–46 ℃ 全部判为正常并呈绿色，与硬件实际一致。
- 全站回归：12 个 LuCI 子页面（网络状态/网络设置/拨号/扫频/锁频/模组设置/升级/
  短信中心/短信设置/服务配置/终端/日志）均无 JS 报错，主色令牌统一生效。
