...
> 包名 `luci-app-mt5700` · 服务/UCI 段 `at-webserver` · 当前版本 **v1.8.0**
...
apk add --allow-untrusted \
  ./aarch64_cortex-a53-luci-app-mt5700-1.8.0-r1.apk \
...
```sh
opkg install ./aarch64_cortex-a53-luci-app-mt5700_1.8.0_aarch64_cortex-a53.ipk
...
> **② 信号质量**卡片增加总评横幅（按木桶原理取最差项，直接回答「能不能用」）、
> 中文主标签 + 英文副标签、每档人话描述；并修正环形仪表弧长量程过窄导致的视觉失真。
...
- **总评横幅**：把四项指标汇总成一句结论 + 可执行建议，如
  「信号很好 · 当前网络质量优秀，适合看高清视频、下载大文件等对带宽要求高的场景」。
...
├── Makefile                         # PKG_NAME=luci-app-mt5700 · PKG_VERSION=1.8.0
...
| 目标系统 | 包格式 | 架构 | 产物示例 |
|:--|:--|:--|:--|
| 主线 snapshot | `.apk` | x86_64 · aarch64_cortex-a53 | `x86_64-luci-app-mt5700-1.8.0-r1.apk` |
| 23.05.5 | `.ipk` | x86_64 · aarch64_cortex-a53 | `x86_64-luci-app-mt5700_1.8.0_x86_64.ipk` |
...
- `main` 推送 → Release tag = `Makefile` 中的 `PKG_VERSION`（当前 `v1.8.0`）  
