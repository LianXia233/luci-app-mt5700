...
- **网络接口**：V4 与 V6 接口在缺失时都会自动创建，两者共用模组同一 USB 网口（OpenWrt 默认配置里的 `wan` / `wan6` 也是这种写法）：
  ...
  | `MT5700Mv6` | `dhcpv6` | **按实际网络状况获取**（`reqaddress=try` / `reqprefix=auto`）：运营商下发就取到，不下发也不影响 IPv4 使用 |

  创建后置 `auto=1`，并在模组网口就绪后带间隔重试 `ifup`（覆盖 30~60s 的冷启动链路）。若系统缺少 `odhcp6c`，会跳过 V6 接口的创建并记录原因。
