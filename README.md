...
> 包名 `luci-app-mt5700` · 服务/UCI 段 `at-webserver` · 当前版本 **v1.3.2**
...
`v1.3.2` 起，速率状态新增显式单位口径字段 `speedUnit`（`'bytes'` / `'kbps'`），
由赋值方声明单位、格式化函数据此选择换算路径，不再隐式假设：

| `speedUnit` | 换算 | 归属来源 |
|:--|:--|:--|
| `'bytes'` | `value × 8` | PDCP 实时速率 |
| `'kbps'` | `value × 1000` | `AT^DSAMBR` 签约速率 |

修复后两个面板均正确显示 `102.40 Mbps`。若后续新增速率来源（如 QoS 协商速率），
只需在赋值处声明一次单位归属即可，不会再产生同类错位。
