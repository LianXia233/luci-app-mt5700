...
return view.extend({
	...
	styleNode: function() {
		...
		'.mt5700m-webui-cta{display:flex;align-items:center;gap:14px;padding:14px 18px;margin-bottom:12px;border-radius:16px;text-decoration:none!important;color:#fff;background:linear-gradient(135deg,#1264d8 0%,#087eae 55%,#07988e 100%);box-shadow:0 10px 26px rgba(14,92,155,.18);cursor:pointer;transition:transform .14s cubic-bezier(.34,1.56,.64,1),box-shadow .2s ease,filter .15s ease;will-change:transform;-webkit-tap-highlight-color:transparent}.mt5700m-webui-cta:hover{transform:translateY(-2px);box-shadow:0 16px 34px rgba(14,92,155,.26);filter:brightness(1.04)}.mt5700m-webui-cta:active{transform:scale(.97);transition-duration:.06s}.mt5700m-webui-cta *{text-decoration:none!important}.mt5700m-webui-cta-icon{display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:10px;background:rgba(255,255,255,.18);font-size:20px;flex:none;transition:transform .2s ease}.mt5700m-webui-cta:hover .mt5700m-webui-cta-icon{transform:scale(1.08) rotate(-4deg)}.mt5700m-webui-cta-text{flex:1;display:flex;flex-direction:column;gap:2px}.mt5700m-webui-cta-text strong{font-size:15px;font-weight:750;letter-spacing:.2px}.mt5700m-webui-cta-text span{font-size:10px;color:rgba(255,255,255,.82);line-height:1.4}.mt5700m-webui-cta b{font-size:22px;font-weight:700;transition:transform .2s ease}.mt5700m-webui-cta:hover b{transform:translate(2px,-2px)}',
	...
	render: function(res) {
		...
		return E('div', { 'class':'mt5700m-page mt-ui-page' }, [
			...
			E('section', { 'class':'mt5700m-hero' }, [
				...
				E('div', { 'class':'mt5700m-hero-meta' }, [ E('span', {}, [ _('Operator'), opInfo.logo ? E('img', { 'class':'mt5700m-op-logo', 'style':'width:16px;height:16px;border-radius:4px;vertical-align:-2px;margin:0 5px 0 0;object-fit:contain', 'src': opInfo.logo, 'alt': operator }) : null, E('strong', {}, operator || '--') ]), E('span', {}, [ _('Network Mode'), E('strong', {}, data.sysmode_detail || data.sysmode || '--') ]), E('span', {}, [ _('Network interface'), E('strong', {}, data.network_interface || '--') ]) ])
				]),
			...
			E('a', { 'class':'mt5700m-webui-cta', 'href':'/5700/', 'target':'_blank', 'rel':'noopener' }, [
				...
				E('span', { 'class':'mt5700m-webui-cta-text' }, [
					...
					E('span', {}, _('独立的 Web 管理界面：实时信号、小区、短信、AT 终端、系统升级等'))
				]),
				E('b', {}, '↗')
			]),
