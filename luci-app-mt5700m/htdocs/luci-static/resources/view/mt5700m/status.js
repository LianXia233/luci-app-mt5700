...
return view.extend({
	...
	styleNode: function() {
		return E('style', {}, [
			...
			'.mt5700m-hero-meta{display:flex;flex-wrap:wrap;gap:7px 18px;margin-top:13px;font-size:11px;color:rgba(255,255,255,.72)}.mt5700m-hero-meta strong{margin-left:5px;color:#fff;font-weight:700}.mt5700m-hero-op{display:inline-flex;align-items:center;gap:6px;margin-left:0}.mt5700m-hero-op img{width:20px;height:20px;border-radius:5px;object-fit:contain;flex:none}.mt5700m-hero-op strong{margin-left:0;font-size:14px;font-weight:750;letter-spacing:.02em}',
		...
		'.mt5700m-webui-cta{display:inline-flex;align-items:center;gap:8px;padding:7px 14px;margin-bottom:12px;border-radius:10px;text-decoration:none!important;color:#fff;background:linear-gradient(135deg,#1a6ddb 0%,#0e8fbc 100%);box-shadow:0 4px 12px rgba(14,92,155,.20);cursor:pointer;transition:transform .14s ease,box-shadow .2s ease;will-change:transform;-webkit-tap-highlight-color:transparent;font-size:0;white-space:nowrap}.mt5700m-webui-cta:hover{transform:translateY(-1px);box-shadow:0 6px 16px rgba(14,92,155,.30)}.mt5700m-webui-cta:active{transform:scale(.97)}.mt5700m-webui-cta *{text-decoration:none!important}.mt5700m-webui-cta-icon{display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:7px;background:rgba(255,255,255,.20);font-size:15px;flex:none}.mt5700m-webui-cta-text strong{font-size:13px;font-weight:700;letter-spacing:.15px}.mt5700m-webui-cta-text span{font-size:10px;color:rgba(255,255,255,.80);margin-left:4px}.mt5700m-webui-cta b{font-size:15px;font-weight:700;margin-left:2px}',
	...
	operatorInfo: function(name) {
		...
		if (n.indexOf('CHINA MOBILE') !== -1 || n.indexOf('CMCC') !== -1)
			return { name: '中国移动', logo: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#0078D4"/><stop offset="100%" stop-color="#00A2E8"/></linearGradient></defs><rect width="40" height="40" rx="9" fill="url(#g)"/><path d="M7 26c0-1 .5-3 2-5s5-4 9-4 7 2 8 5c-.5-.5-2-2-5-3s-8-1-11 1-3 3-3 6z" fill="#fff"/><circle cx="22" cy="21" r="3.5" fill="#fff"/><path d="M9 13c1.5-3 5-5.5 10-5s8 3 9 6c-1.5-1.5-4-3-8-3s-9 2-11 2z" fill="#FFDE00"/></svg>') };
		if (n.indexOf('CHINA UNICOM') !== -1 || n.indexOf('UNICOM') !== -1)
			return { name: '中国联通', logo: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"><rect width="40" height="40" rx="9" fill="#E60012"/><path d="M20 8C13.4 8 8 13.4 8 20c0 3.8 1.7 7.2 4.4 9.5L20 32l7.6-2.5C30.3 27.2 32 23.8 32 20c0-6.6-5.4-12-12-12zm0 4c4.4 0 8 3.6 8 8s-3.6 8-8 8-8-3.6-8-8 3.6-8 8-8z" fill="#fff" opacity=".95"/><path d="M20 14c-3.3 0-6 2.7-6 6s2.7 6 6 6 6-2.7 6-6-2.7-6-6-6z" fill="#fff"/></svg>') };
		if (n.indexOf('CHINA TELECOM') !== -1 || n.indexOf('TELECOM') !== -1)
			return { name: '中国电信', logo: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"><defs><linearGradient id="t" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color:#0078D4"/><stop offset="100%" stop-color="#009FE8"/></linearGradient></defs><rect width="40" height="40" rx="9" fill="url(#t)"/><g stroke="#fff" stroke-linecap="round" fill="none"><path d="M8 22Q16 12 26 18T35 15" stroke-width="3.2"/><path d="M8 27Q16 17 28 23T35 20" stroke-width="2.4" opacity=".75"/><path d="M8 17Q16 9 24 14T33 11" stroke-width="1.8" opacity=".5"/></g></svg>') };
		if (n.indexOf('BROADNET') !== -1 || n.indexOf('GBA') !== -1 || n.indexOf('CHINA BROADCASTING') !== -1 || n.indexOf('CBN') !== -1)
			return { name: '中国广电', logo: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"><defs><linearGradient id="b" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#7C3AED"/><stop offset="100%" stop-color="#F97316"/></linearGradient></defs><rect width="40" height="40" rx="9" fill="url(#b)"/><text x="20" y="25.5" text-anchor="middle" fill="#fff" font-size="17" font-weight="800" font-family="-apple-system,BlinkMacSystemFont,\'PingFang SC\',\'Microsoft YaHei\',sans-serif">广</text></svg>') };
	...
	render: function(res) {
		...
		return E('div', { 'class':'mt5700m-page mt-ui-page' }, [
			...
			E('a', { 'class':'mt5700m-webui-cta', 'href':'/5700/', 'target':'_blank', 'rel':'noopener' }, [
				...
				E('span', { 'class':'mt5700m-webui-cta-text' }, [
					E('strong', {}, _('打开模块 WebUI')),
					E('span', {}, _('信号·小区·短信·AT终端'))
				]),
				E('b', {}, '↗')
			]),
