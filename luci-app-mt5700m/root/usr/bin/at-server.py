...
DEFAULT_CONFIG = {
    "AT_CONFIG": {
        "TYPE": "UBUS",  # 可选值: "NETWORK"、"SERIAL" 或 "UBUS"（经 ubus-at-daemon 转发，推荐，与 luci-app-mt5700m 共享 AT 通道）
        "NETWORK": {
            "HOST": "192.168.8.1",
            "PORT": 20249,
            "TIMEOUT": 10
        ...
        "UBUS": {
            "AT_PORT": "",   # 留空则自动探测 MT5700M PCUI 串口
            "TIMEOUT": 10
        }
    },
    "NOTIFICATION_CONFIG": {
...
def load_config():
    ...
    try:
        ...
        # 读取连接类型（默认 UBUS：经 ubus-at-daemon 转发，与 luci-app-mt5700m 共享 AT 通道）
        conn_type = uci_data.get('connection_type', 'UBUS')
        ...
        # 读取 UBUS 配置
        if conn_type == 'UBUS':
            at_port = uci_data.get('ubus_at_port', '')
            timeout = int(uci_data.get('ubus_timeout', '10'))
            config['AT_CONFIG']['UBUS']['AT_PORT'] = at_port
            config['AT_CONFIG']['UBUS']['TIMEOUT'] = timeout
            logger.info(f"配置加载: UBUS 连接 (at_port={at_port or '自动探测'}, 超时: {timeout}秒)")
    ...
    async def _do_send(self, sender: str, content: str, is_memory_full: bool = False):
        ...
        while retries < self.max_retries:
            try:
                ...
                raise Exception(f"HTTP错误 {status}: {response_text}")
...
class UbusATConnection(ATConnection):
    """UBUS AT连接实现：经 ubus-at-daemon 转发 AT 命令。

    OpenWrt 上 PCUI 串口通常已被 ubus-at-daemon 独占，直接打开串口或
    连接网络 AT 端口都会失败/冲突。该实现调用
    `ubus call at-daemon sendat` 让 daemon 统一串行化所有 AT 请求，
    与 luci-app-mt5700m 的其它组件安全共享同一 AT 通道。
    """

    def __init__(self, at_port: str = "", timeout: int = 10):
        super().__init__()
        self.at_port = at_port
        self.timeout = max(1, int(timeout))

    def _detect_at_port(self) -> str:
        """探测 MT5700M PCUI 串口（bInterfaceClass:Sub:Proto = ff:06:12）"""
        import glob as _glob
        try:
            for tty in sorted(_glob.glob('/sys/class/tty/ttyUSB*')):
            ...
            logger.debug(f"PCUI 串口探测失败: {e}")
        return '/dev/ttyUSB1'  # MT5700M 默认 PCUI 口
    ...
    async def connect(self) -> bool:
        ...
        try:
            if not self.at_port:
                self.at_port = self._detect_at_port()
            # 验证 at-daemon 可用且 AT 通道就绪
            result = await asyncio.to_thread(
                subprocess.run,
                ['ubus', 'call', 'at-daemon', 'sendat',
                 json.dumps({'at_port': self.at_port, 'at_cmd': 'AT', 'timeout': self.timeout})],
            ...
            if result.returncode == 0:
                ...
                if data.get('status') == 'success':
                    self.is_connected = True
                    logger.info(f"已通过 ubus at-daemon 连接 AT 通道 (at_port={self.at_port})")
                    return True
            logger.warning(f"UBUS AT 连接失败: rc={result.returncode} out={(result.stdout or result.stderr or '')[:200]}")
            return False
        except Exception as e:
            logger.warning(f"UBUS AT 连接失败: {e}")
            return False
    ...
    @handle_connection_error
    async def send(self, data: bytes) -> int:
        # ubus 模式为请求/响应式，send/receive 不单独使用
        return len(data)
    ...
    async def send_command(self, command: str) -> bytearray:
        """通过 ubus at-daemon 发送 AT 命令（请求/响应式，整体重写）"""
        import subprocess
        try:
            if not self.is_connected:
                if not await self.connect():
            ...
            async with self._command_lock:
                ...
                result = await asyncio.to_thread(
                    subprocess.run,
                    ['ubus', 'call', 'at-daemon', 'sendat', payload],
                    capture_output=True, text=True, timeout=self.timeout + 5
                )
                self._last_command_time = time.time()

                if result.returncode != 0:
                    logger.error(f"ubus sendat 调用失败: {result.stderr.strip()[:200]}")
                    self.is_connected = False
                    return bytearray()
                ...
                if not response:
                    ...
                    logger.warning(f"ubus sendat 无响应 (status={status}, cmd={cmd[:60]})")
                    if status != 'success':
                        self.is_connected = False
                    return bytearray()

                return bytearray(response.encode('utf-8', errors='ignore'))

        except KeyboardInterrupt:
            raise
        except Exception as e:
            self.is_connected = False
            logger.error(f"UBUS 命令发送失败: {e}")
            await asyncio.sleep(1)
            return bytearray()
...
class ATClient:
    def __init__(self):
        ...
        if self.connection_type == "UBUS":
            self.connection = UbusATConnection(
                at_port=AT_CONFIG.get("UBUS", {}).get("AT_PORT", ""),
                timeout=AT_CONFIG.get("UBUS", {}).get("TIMEOUT", 10)
            )
...
class WebSocketServer:
    ...
    async def broadcast(self, message: dict):
        ...
        for websocket in self._active_connections.copy():
            try:
                # 检查连接是否仍然活跃
                if getattr(websocket, 'closed', False) or not getattr(websocket, 'open', True):
...
async def main():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"IPv6 WebSocket 绑定失败 (已忽略，仅使用 IPv4): {e}")
            server_v6 = None
